...
class TwoViewController: UIViewController {
    ...
    private lazy var noSoundBtn:UIButton = {
        ...
        btn.addTarget(self, action: #selector(TwoViewController.noSound), for: .touchUpInside)
        return btn
    }()
    private lazy var playBtn:UIButton = {
        let btn = UIButton.init(frame: CGRect.zero)
        btn.setImage(#imageLiteral(resourceName: "play"), for: .normal)
        btn.addTarget(self, action: #selector(TwoViewController.playAction), for: .touchUpInside)
        return btn
    }()
    private lazy var nextBtn:UIButton = {
        let btn = UIButton.init(frame: CGRect.zero)
        btn.setImage(#imageLiteral(resourceName: "next"), for: .normal)
        btn.addTarget(self, action: #selector(TwoViewController.nextAction), for: .touchUpInside)
        return btn
    }()
